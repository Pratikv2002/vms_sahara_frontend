...
export default function Dashboard() {
  ...
  const getWeekVisitCount = ()=>{
    try {
      axios.get("/get-lastseven-day-visit").then((response)=>{
        if(response.data.status){
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <div>
            <span style={{ color: "#184CCF", fontSize: "25px" }}>{todayVisitCount?.approvedCount}</span>
            <span
              style={{ color: "#999999", fontSize: "25px", margin: "0px 5px" }}
            >
              |
            </span>
            <span style={{ color: "#FF8039", fontSize: "25px" }}>{todayVisitCount?.checkoutCount}</span>
          </div>
        </div>
        <CreateVisitTiles
          FirstIcon={IoMdAdd}
      ...
      <div style={{ flexWrap:'wrap',display:'flex',padding:"5px 15px", backgroundColor:"white",marginTop:"25px", borderRadius:"5px", boxShadow: "0px 1px 3px grey" ,height:"100%",justifyContent:"space-between"}}>
        <div>
          <h2>Today's Stats</h2>
          <div>
      ...
      >
        ...
        <div
          style={{
            ...
            background: 'linear-gradient(161deg, #81ABFF 0%, #7E4FE2 100%)', // Set color of the rectangle to red
...
var data = weekVisitCount &&  formatDateArray(weekVisitCount);
